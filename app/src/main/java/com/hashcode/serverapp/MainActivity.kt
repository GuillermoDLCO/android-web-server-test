...
class MainActivity : AppCompatActivity() {
    ...
    private fun startServer(port: Int) {
        try {
            mHttpServer = HttpServer.create(InetSocketAddress("0.0.0.0",port), 0)
            mHttpServer!!.executor = Executors.newCachedThreadPool()

            mHttpServer!!.createContext("/", rootHandler)
            mHttpServer!!.createContext("/reboot", rebootHandler)
            mHttpServer!!.createContext("/update", updateHandler)
    ...
    private val rootHandler = HttpHandler { exchange ->
        run {
            ...
            when (exchange!!.requestMethod) {
                "GET" -> {
                    sendResponse(exchange, "Welcome to my web server")
                }
                "POST" -> {
                    sendResponse(exchange, "Welcome to my web server")
                }
    ...
    // Handler for reboot endpoint
    private val rebootHandler = HttpHandler { exchange ->
        run {
            // Get request method
            when (exchange!!.requestMethod) {
                "POST" -> {
                    ...
                    sendResponse(exchange, jsonBody.toString(2))
                }
    ...
    private val updateHandler = HttpHandler { exchange ->
        run {
            // Get request method
            when (exchange!!.requestMethod) {
                "POST" -> {
    ...
    private val messageHandler = HttpHandler { httpExchange ->
        run {
            when (httpExchange!!.requestMethod) {
                "GET" -> {
                    // Get all messages
                    sendResponse(httpExchange, "Would be all messages json")
                }
                "POST" -> {
